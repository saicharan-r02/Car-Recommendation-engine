"""
Database module for Sports Car Recommendation Engine.
Uses SQLite and SQLAlchemy to provide a dynamic car inventory,
auto-seeding from CSV, user query audit logs, and favorite/wishlist storage.
"""
...
from sqlalchemy import (
    create_engine, Column, Integer, Float, String, DateTime, ForeignKey, func
)
...
class Car(Base):
    ...
    zero_to_sixty = Column(Float, nullable=False, index=True)  # 0-60 MPH in seconds
    price_usd = Column(Float, nullable=False, index=True)      # Price in USD
...
def init_db():
    ...
    try:
        ...
        session.close()
    print(f"[OK] Car Recommendation database initialized at: {DB_PATH}")
